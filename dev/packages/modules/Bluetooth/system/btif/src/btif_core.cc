...
// ***** ADICAO *****
void btif_dm_report_rssi_alert(const RawAddress& bd_addr) {
   // Em vez de acessar bt_hal_cbacks diretamente, usamos a interface de eventos.
   // Isso garante que o callback seja chamado corretamente independente da arquitetura.
   
   GetInterfaceToProfiles()->events->invoke_acl_state_changed_cb(
             BT_STATUS_SUCCESS, 
             bd_addr, 
             (bt_acl_state_t)3,        // State 3 (Alerta)
             (int)BT_TRANSPORT_BR_EDR, 
             HCI_SUCCESS,              
             (bt_conn_direction_t)0,
             0
             );
}
// *************************
