...
#include "stack/include/btm_api.h" // Para BTM_ReadRssi e tBTM_STATUS
#include "stack/acl/acl.h"      // Para tACL_CONN e btm_acl_for_bda
...
static jint getConnectedDeviceRssiNative(JNIEnv* env, [[maybe_unused]] jobject obj, jbyteArray address) {
    ...
    if (!sCallbackEnv.valid()) return -127; // Valor de erro

    // Converte jbyteArray para RawAddress
    RawAddress bd_addr;
    jbyte* addr = env->GetByteArrayElements(address, NULL);
    if (addr == nullptr) {
      log::error("SHAKKA_LOG JNI: Falha ao obter bytes do endereço");
      return -127; // Erro
    }
    bd_addr.FromOctets(reinterpret_cast<const uint8_t*>(addr));
    env->ReleaseByteArrayElements(address, addr, JNI_ABORT); // Libera memoria

    // encontra a conexao ACL
    tACL_CONN* p_acl = btm_acl_for_bda(bd_addr, BT_TRANSPORT_BR_EDR);

    // retorna o valor cacheado ou erro
    if (p_acl != nullptr && p_acl->in_use) {
        log::info("SHAKKA_LOG JNI: Retornando RSSI cacheado %d para %s",
                  p_acl->last_known_rssi, bd_addr.ToString().c_str());
        return static_cast<jint>(p_acl->last_known_rssi);
    }

    log::warn("SHAKKA_LOG JNI: Nao foi encontrada conexao ACL ativa para %s",
              bd_addr.ToString().c_str());
    return -127; // Valor de erro
}

// Funcao JNI para obter o ultimo motivo de desconexao
static jint getDisconnectionReasonNative(JNIEnv* env, [[maybe_unused]] jobject obj, jbyteArray address) {
    CallbackEnv sCallbackEnv(__func__);
    if (!sCallbackEnv.valid()) return -1; // Erro generico

    RawAddress bd_addr;
    jbyte* addr = env->GetByteArrayElements(address, NULL);
    if (addr == nullptr) {
      log::error("SHAKKA_LOG JNI: Falha ao obter bytes do endereço (getReason)");
      return -1;
    }
    bd_addr.FromOctets(reinterpret_cast<const uint8_t*>(addr));
    ...
    if (p_acl != nullptr) { // Nao precisa checar in_use, queremos o ultimo motivo
        log::info("SHAKKA_LOG JNI: Retornando motivo %d para %s",
                  p_acl->last_disconnect_reason, bd_addr.ToString().c_str());
        return static_cast<jint>(p_acl->last_disconnect_reason);
    }

    log::warn("SHAKKA_LOG JNI: Nao foi encontrada conexao ACL (getReason) para %s",
              bd_addr.ToString().c_str());
    return -1; // Indica que nao achou o registro
}
// ====================
...
int register_com_android_bluetooth_btservice_AdapterService(JNIEnv* env) {
  const JNINativeMethod methods[] = {
      ...
      {"pbapPseDynamicVersionUpgradeIsEnabledNative", "()Z",
       ...
       {"getConnectedDeviceRssiNative", "([B)I", (void*)getConnectedDeviceRssiNative}, // shakka
       {"getDisconnectionReasonNative", "([B)I", (void*)getDisconnectionReasonNative}, // shakka
  };
