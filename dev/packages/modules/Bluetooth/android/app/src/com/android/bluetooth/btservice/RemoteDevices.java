...
final class RemoteDevices {
 ...
 private
  static final String TAG = "BluetoothRemoteDevices";

  // Maximum number of device properties to remember
 private
  static final int MAX_DEVICE_QUEUE_SIZE = 200;
 ...
 private
  ArrayList<BluetoothDevice> mSdpTracker;
 private
  final Object mObject = new Object();

 private
  static final int UUID_INTENT_DELAY = 6000;
 private
  static final int MESSAGE_UUID_INTENT = 1;
 private
  static final String LOG_SOURCE_DIS = "DIS";

 private
  final HashMap<String, DeviceProperties> mDevices;
 private
  final HashMap<String, String> mDualDevicesMap;
 private
  ArrayDeque<String> mDeviceQueue;

  /**
   * Bluetooth HFP v1.8 specifies the Battery Charge indicator of AG can take
   * values from
   * {@code 0} to {@code 5}, but it does not specify how to map the values back
   * to percentages. The following mapping is used:
   *   - Level 0:                 0%
   ...
   */
 private
  static final int HFP_BATTERY_CHARGE_INDICATOR_0 = 0;
 private
  static final int HFP_BATTERY_CHARGE_INDICATOR_1 = 13;
 private
  static final int HFP_BATTERY_CHARGE_INDICATOR_2 = 38;
 private
  static final int HFP_BATTERY_CHARGE_INDICATOR_3 = 63;
 private
  static final int HFP_BATTERY_CHARGE_INDICATOR_4 = 88;
 private
  static final int HFP_BATTERY_CHARGE_INDICATOR_5 = 100;

 private
  final Handler mHandler;
 private
  class RemoteDevicesHandler extends Handler {
    /**
     * Handler must be created from an explicit looper to avoid threading
     * ambiguity
     * @param looper The looper that this handler should be executed on
     */
    RemoteDevicesHandler(Looper looper) { super(looper); }
   ...
   */
 private
  final Predicate<BluetoothDevice> mLocationDenylistPredicate = (device)->{
    final MacAddress parsedAddress = MacAddress.fromString(device.getAddress());
    if (mAdapterService.getLocationDenylistMac().test(
            parsedAddress.toByteArray())) {
  ...
  /**
   * Clean up should be called when this object is no longer needed, must be
   * called after init()
   */
  @RequiresPermission(
      android.Manifest.permission.BLUETOOTH_CONNECT) void cleanup() {
   ...
   */
  @RequiresPermission(
      android.Manifest.permission.BLUETOOTH_CONNECT) void reset() {
    if (mSdpTracker != null) {
      mSdpTracker.clear();
    }
    ...
    synchronized(mDevices) {
      if (mDevices != null) {
        ...
        mDevices.forEach((address, deviceProperties)->{
          ...
          debugLog("reset(): address=" + address +
  ...
  @VisibleForTesting DeviceProperties addDeviceProperties(byte[] address) {
    synchronized(mDevices) {
  ...
  class DeviceProperties {
   ...
   private
    byte[] mAddress;
   private
    String mIdentityAddress;
   private
    boolean mIsConsolidated = false;
   private
    int mBluetoothClass = BluetoothClass.Device.Major.UNCATEGORIZED;
   private
    int mBredrConnectionHandle = BluetoothDevice.ERROR;
   private
    int mLeConnectionHandle = BluetoothDevice.ERROR;
   private
    short mRssi;
   private
    String mAlias;
   ...
   private
    boolean mIsBondingInitiatedLocally;
   private
    int mBatteryLevel = BluetoothDevice.BATTERY_LEVEL_UNKNOWN;
   private
    boolean mIsCoordinatedSetMember;
   private
    int mAshaCapability;
   private
    int mAshaTruncatedHiSyncId;
   private
    String mModelName;
   ...
   private
    BluetoothSinkAudioPolicy mAudioPolicy;

    DeviceProperties() { mBondState = BluetoothDevice.BOND_NONE; }

    /**
     * @return the mName
     */
    String getName() {
      synchronized(mObject) { return mName; }
    }
    ...
    void setName(String name) {
      synchronized(mObject) { this.mName = name; }
    }
    ...
    String getIdentityAddress() {
      synchronized(mObject) { return mIdentityAddress; }
    }
    ...
    void setIdentityAddress(String identityAddress) {
      synchronized(mObject) { this.mIdentityAddress = identityAddress; }
    }
    ...
    boolean isConsolidated() {
      synchronized(mObject) { return mIsConsolidated; }
    }
    ...
    void setIsConsolidated(boolean isConsolidated) {
      synchronized(mObject) { this.mIsConsolidated = isConsolidated; }
    }
    ...
    int getBluetoothClass() {
      synchronized(mObject) { return mBluetoothClass; }
    }
    ...
    void setBluetoothClass(int bluetoothClass) {
      synchronized(mObject) { this.mBluetoothClass = bluetoothClass; }
    }
    ...
    void setConnectionHandle(int connectionHandle, int transport) {
      synchronized(mObject) {
        if (transport == BluetoothDevice.TRANSPORT_BREDR) {
          ...
          errorLog("setConnectionHandle() unexpected transport value " +
                   transport);
        }
      }
    }
     ...
     */
    ParcelUuid[] getUuids() {
      synchronized(mObject) { return mUuids; }
    }
    ...
    void setUuids(ParcelUuid[] uuids) {
      synchronized(mObject) { this.mUuids = uuids; }
    }
     ...
     */
    byte[] getAddress() {
      synchronized(mObject) { return mAddress; }
    }
    ...
    void setAddress(byte[] address) {
      synchronized(mObject) { this.mAddress = address; }
    }
    ...
    BluetoothDevice getDevice() {
      synchronized(mObject) { return mDevice; }
    }
    ...
    void setDevice(BluetoothDevice device) {
      synchronized(mObject) { this.mDevice = device; }
    }
    ...
    void setRssi(short rssi) {
      synchronized(mObject) { this.mRssi = rssi; }
    }
     ...
     */
    int getDeviceType() {
      synchronized(mObject) { return mDeviceType; }
    }
    ...
    void setDeviceType(int deviceType) {
      synchronized(mObject) { this.mDeviceType = deviceType; }
    }
     ...
     */
    String getAlias() {
      synchronized(mObject) { return mAlias; }
    }
    ...
    void setAlias(BluetoothDevice device, String mAlias) {
      synchronized(mObject) {
        this.mAlias = mAlias;
        mAdapterService.setDevicePropertyNative(
            mAddress, AbstractionLayer.BT_PROPERTY_REMOTE_FRIENDLY_NAME,
            mAlias.getBytes());
    ...
    void setBondState(int newBondState) {
      synchronized(mObject) {
        ...
        if (newBondState == BluetoothDevice.BOND_NONE) {
          /* Clearing the Uuids local copy when the device is unpaired. If not
          cleared, cachedBluetoothDevice issued a connect using the local cached
          copy of uuids, without waiting for the ACTION_UUID intent. This was
          resulting in multiple calls to connect().*/
          mUuids = null;
          mAlias = null;
        }
      }
     ...
     */
    int getBondState() {
      synchronized(mObject) { return mBondState; }
    }
    ...
    boolean isBondingInitiatedLocally() {
      synchronized(mObject) { return mIsBondingInitiatedLocally; }
    }
     ...
     */
    int getBatteryLevel() {
      synchronized(mObject) { return mBatteryLevel; }
    }
    ...
    void setBatteryLevel(int batteryLevel) {
      synchronized(mObject) { this.mBatteryLevel = batteryLevel; }
    }
    ...
    boolean isCoordinatedSetMember() {
      synchronized(mObject) { return mIsCoordinatedSetMember; }
    }
    ...
    int getAshaCapability() {
      synchronized(mObject) { return mAshaCapability; }
    }

    void setAshaCapability(int ashaCapability) {
      synchronized(mObject) { this.mAshaCapability = ashaCapability; }
    }
    ...
    int getAshaTruncatedHiSyncId() {
      synchronized(mObject) { return mAshaTruncatedHiSyncId; }
    }
   ...
   public
    void setHfAudioPolicyForRemoteAg(BluetoothSinkAudioPolicy policies) {
      mAudioPolicy = policies;
    }

   public
    BluetoothSinkAudioPolicy getHfAudioPolicyForRemoteAg() {
      return mAudioPolicy;
    }

   public
    void setModelName(String modelName) { mModelName = modelName; }

    /**
     * @return the mModelName
     */
    String getModelName() {
      synchronized(mObject) { return mModelName; }
    }
  }

  private void
  sendUuidIntent(BluetoothDevice device, DeviceProperties prop) {
    ...
    MetricsLogger.getInstance().cacheCount(BluetoothProtoEnums.SDP_SENT_UUID,
                                           1);
  ...
  /**
   * When bonding is initiated to remote device that we have never seen, i.e Out
   * Of Band pairing, we must add device first before setting it's properties.
   * This is a helper method for doing that.
   */
   ...
   */
  @VisibleForTesting void updateBatteryLevel(BluetoothDevice device,
                                             int batteryLevel) {
    ...
    Log.d(TAG, "Updated device " + device + " battery level to " +
                   batteryLevel + "%");
  }

  /**
   * Reset battery level property to {@link
   * BluetoothDevice#BATTERY_LEVEL_UNKNOWN} for a device
   * @param device device whose battery level property needs to be reset
   */
  @VisibleForTesting void resetBatteryLevel(BluetoothDevice device) {
    if (device == null) {
    ...
    synchronized(mObject) {
      if (deviceProperties.getBatteryLevel() ==
    ...
    sendBatteryLevelChangedBroadcast(device,
                                     BluetoothDevice.BATTERY_LEVEL_UNKNOWN);
    Log.d(TAG, "Reset battery level, device=" + device);
  }

 private
  void sendBatteryLevelChangedBroadcast(BluetoothDevice device,
                                        int batteryLevel) {
  ...
  /**
   * Converts HFP's Battery Charge indicator values of {@code 0 -- 5} to an
   * integer percentage.
   */
  @VisibleForTesting static int batteryChargeIndicatorToPercentge(
      int indicator) {
    int percent;
    switch (indicator) {
      case 5:
    ...
    Log.d(TAG, "Battery charge indicator: " + indicator +
                   "; converted to: " + percent + "%");
    return percent;
  }

 private
  static boolean areUuidsEqual(ParcelUuid[] uuids1, ParcelUuid[] uuids2) {
  ...
  void devicePropertyChangedCallback(byte[] address, int[] types,
                                     byte[][] values) {
    Intent intent;
    byte[] val;
    int type;
        ...
        synchronized(mObject) {
          ...
          switch (type) {
            ...
            case AbstractionLayer.BT_PROPERTY_REMOTE_FRIENDLY_NAME:
              ...
              debugLog("Remote device alias is: " +
                       deviceProperties.getAlias());
              break;
            ...
            case AbstractionLayer.BT_PROPERTY_REMOTE_MODEL_NUM:
              ...
              BluetoothStatsLog.write(
                  ...
                  BluetoothProtoEnums.DEVICE_INFO_INTERNAL, LOG_SOURCE_DIS,
  ...
  void deviceFoundCallback(byte[] address) {
    ...
    boolean restrict_device_found = SystemProperties.getBoolean(
        "bluetooth.restrict_discovered_device.enabled", false);
    if (restrict_device_found &&
        (deviceProp.mName == null || deviceProp.mName.isEmpty())) {
    ...
    final ArrayList<DiscoveringPackage> packages =
        mAdapterService.getDiscoveringPackages();
      ...
      for (DiscoveringPackage pkg : packages) {
        ...
        if (pkg.getPermission() != null) {
          mAdapterService.sendBroadcastMultiplePermissions(
              intent, new String[]{BLUETOOTH_SCAN, pkg.getPermission()},
  ...
  void addressConsolidateCallback(byte[] mainAddress, byte[] secondaryAddress) {
    ...
    Log.d(TAG, "addressConsolidateCallback device: " + device +
                   ", secondaryAddress:" +
                   Utils.getRedactedAddressStringFromByte(secondaryAddress));
    ...
    mDualDevicesMap.put(deviceProperties.getIdentityAddress(),
  ...
  void leAddressAssociateCallback(byte[] mainAddress, byte[] secondaryAddress) {
    ...
    Log.d(TAG, "leAddressAssociateCallback device: " + device +
                   ", secondaryAddress:" +
                   Utils.getRedactedAddressStringFromByte(secondaryAddress));

    DeviceProperties deviceProperties = getDeviceProperties(device);
    deviceProperties.mIdentityAddress =
        Utils.getAddressStringFromByte(secondaryAddress);
  }
  ...
  @RequiresPermission(allOf =
                          ...
                          }) void aclStateChangeCallback(int status,
                                                         byte[] address,
                                                         int newState,
    ...
    if (newState == AbstractionLayer.BT_ACL_STATE_CONNECTED) {
      ...
      if (state == BluetoothAdapter.STATE_ON ||
        ...
        Log.d(
            TAG,
            "aclStateChangeCallback() - BluetoothDevice.ACTION_ACL_CONNECTED");
      } else if (state == BluetoothAdapter.STATE_BLE_ON ||
                 state == BluetoothAdapter.STATE_BLE_TURNING_ON) {
        intent = new Intent(BluetoothAdapter.ACTION_BLE_ACL_CONNECTED);
        Log.d(TAG,
              "aclStateChangeCallback() - "
              "BluetoothDevice.ACTION_BLE_ACL_CONNECTED");
      }
      ...
      deviceProperties.setConnectionHandle(BluetoothDevice.ERROR,
                                           transportLinkType);
      if (device.getBondState() == BluetoothDevice.BOND_BONDING) {
        ...
        Log.d(
            TAG,
            "aclStateChangeCallback() - BluetoothDevice.ACTION_PAIRING_CANCEL");
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
      ...
      if (state == BluetoothAdapter.STATE_ON ||
        ...
        Log.d(TAG,
              "aclStateChangeCallback() - "
              "BluetoothDevice.ACTION_BLE_ACL_DISCONNECTED");
      }
      ...
      debugLog("aclStateChangeCallback: Adapter State: " +
               BluetoothAdapter.nameForState(state) + " Disconnected: " +
               device + " transportLinkType: " + transportLinkType +
               " hciReason: " + hciReason);
    }
    ...
    BluetoothStatsLog.write(
        ...
        mAdapterService.obfuscateAddress(device), connectionState, metricId,
        transportLinkType);

    BluetoothClass deviceClass = device.getBluetoothClass();
    int classOfDevice =
        deviceClass == null ? 0 : deviceClass.getClassOfDevice();
    BluetoothStatsLog.write(
    ...
    if (intent != null) {
      ...
      synchronized(mAdapterService.getBluetoothConnectionCallbacks()) {
        ...
        for (IBluetoothConnectionCallback callback :
             bluetoothConnectionCallbacks) {
          try {
            if (connectionState == BluetoothAdapter.STATE_CONNECTED) {
              callback.onDeviceConnected(device);
            } else {
              Log.d(TAG, "aclStateChangeCallback() - disconnected due to %d",
                    hciReason);
              if (hciReason == 0x08) {
                Log.d(TAG,
                      "aclStateChangeCallback() - disconnected due to timeout "
                      "reason" +
                          device);
                intent.putExtra(
                    "android.bluetooth.device.extra.disconnect_reason", 1);
              }
            ...
            Log.e(TAG,
                  "RemoteException in calling IBluetoothConnectionCallback");
          }
        }
      }
    } else {
  ...
  void fetchUuids(BluetoothDevice device, int transport) {
    ...
    // If no UUIDs are cached and the device is bonding, wait for SDP after the
    // device is bonded
    DeviceProperties deviceProperties = getDeviceProperties(device);
   ...
   * @param intent must be {@link
   * BluetoothHeadset#ACTION_CONNECTION_STATE_CHANGED} intent
   */
  @VisibleForTesting void onHeadsetConnectionStateChanged(Intent intent) {
    ...
    if (intent.getIntExtra(BluetoothProfile.EXTRA_STATE,
                           BluetoothProfile.STATE_DISCONNECTED) ==
            BluetoothProfile.STATE_DISCONNECTED &&
  ...
  @VisibleForTesting void onHfIndicatorValueChanged(Intent intent) {
    BluetoothDevice device =
        intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
    if (device == null) {
    ...
    int indicatorId =
        intent.getIntExtra(BluetoothHeadset.EXTRA_HF_INDICATORS_IND_ID, -1);
    int indicatorValue =
        intent.getIntExtra(BluetoothHeadset.EXTRA_HF_INDICATORS_IND_VALUE, -1);
   ...
   */
  @VisibleForTesting void onVendorSpecificHeadsetEvent(Intent intent) {
    BluetoothDevice device =
        intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
    if (device == null) {
   ...
   * @return Battery level in percents, [0-100], {@link
   * BluetoothDevice#BATTERY_LEVEL_UNKNOWN} when there is an error parsing the
   * arguments
   */
  @VisibleForTesting static int getBatteryLevelFromAppleBatteryVsc(
      Object[] args) {
    if (args.length == 0) {
    ...
    if (args[0] instanceof Integer) {
      ...
      Log.w(TAG,
            "getBatteryLevelFromAppleBatteryVsc() error parsing number of "
            "arguments");
      return BluetoothDevice.BATTERY_LEVEL_UNKNOWN;
    }
    if (args.length != (numKvPair * 2 + 1)) {
      Log.w(TAG,
            "getBatteryLevelFromAppleBatteryVsc() number of arguments does not "
            "match");
      return BluetoothDevice.BATTERY_LEVEL_UNKNOWN;
    }
    int indicatorType;
    ...
    for (int i = 0; i < numKvPair; ++i) {
      ...
      if (indicatorTypeObj instanceof Integer) {
        ...
        Log.w(TAG,
              "getBatteryLevelFromAppleBatteryVsc() error parsing indicator "
              "type");
        return BluetoothDevice.BATTERY_LEVEL_UNKNOWN;
      }
      if (indicatorType !=
   ...
   * arguments
   */
  @VisibleForTesting static int getBatteryLevelFromXEventVsc(Object[] args) {
    if (args.length == 0) {
    ...
    if (!eventName.equals(
      ...
      infoLog("getBatteryLevelFromXEventVsc() skip none BATTERY event: " +
              eventName);
      return BluetoothDevice.BATTERY_LEVEL_UNKNOWN;
    }
    if (args.length != 5) {
    ...
    if (batteryLevel < 0 || numberOfLevels <= 1 ||
        batteryLevel > numberOfLevels) {
      Log.w(TAG,
            "getBatteryLevelFromXEventVsc() wrong event value, batteryLevel=" +
                String.valueOf(batteryLevel) +
                ", numberOfLevels=" + String.valueOf(numberOfLevels));
      return BluetoothDevice.BATTERY_LEVEL_UNKNOWN;
    }
    return batteryLevel * 100 / (numberOfLevels - 1);
  }

  @VisibleForTesting boolean hasBatteryService(BluetoothDevice device) {
    BatteryService batteryService = BatteryService.getBatteryService();
   ...
   * @param intent must be {@link
   * BluetoothHeadsetClient#ACTION_CONNECTION_STATE_CHANGED} intent
   */
  @VisibleForTesting void onHeadsetClientConnectionStateChanged(Intent intent) {
    BluetoothDevice device =
        intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
    if (device == null) {
      Log.e(TAG,
            "onHeadsetClientConnectionStateChanged() remote device is null");
      return;
    }
    if (intent.getIntExtra(BluetoothProfile.EXTRA_STATE,
                           BluetoothProfile.STATE_DISCONNECTED) ==
  ...
  @VisibleForTesting void onAgIndicatorValueChanged(Intent intent) {
    BluetoothDevice device =
        intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
    if (device == null) {
    ...
    if (intent.hasExtra(BluetoothHeadsetClient.EXTRA_BATTERY_LEVEL)) {
      int batteryLevel =
          intent.getIntExtra(BluetoothHeadsetClient.EXTRA_BATTERY_LEVEL, -1);
 ...
 private
  static void errorLog(String msg) { Log.e(TAG, msg); }
 ...
 private
  static void warnLog(String msg) { Log.w(TAG, msg); }
}
